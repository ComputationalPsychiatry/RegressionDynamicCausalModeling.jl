function rigid_inversion(
    rdcm::BiLinearRigidRdcm, X::Matrix{Float64}, Y::Matrix{Float64}, opt::Options
)
    ...
    idx = [dcm.a reshape(dcm.b,nr,nr*nu) dcm.c conf_weight_idx]
    ...
    return RigidOutput(
        sum(F_all), F_all, iter_all, a_all, b_all, m_all, Σ_all, "tapas_rdcm_rigid_B"
    )
end
